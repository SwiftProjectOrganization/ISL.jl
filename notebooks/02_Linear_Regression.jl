...
md" ## Chapter 2 - ISL: Linear regerssion."
...
advertising = CSV.read(expanduser("~/.julia/dev/ISL/data/Advertising.csv"), DataFrame)

# ╔═╡ ee0e953d-80f5-49f9-8e89-f93b957cb421
md" ##### Use GLM."

# ╔═╡ 1578c482-eac6-4449-89ce-ed2372afe8d2
ad_lm = lm(@formula(sales ~ TV + radio + newspaper), advertising)

# ╔═╡ f91514f0-217f-4a0d-b8ba-425705b984e0
residuals(ad_lm)

# ╔═╡ 2e7f6ada-e816-486a-a891-f11335bcf62f
std(residuals(ad_lm))
...
let
    ...
    ylabel = "Sales"
    let
        title = "Linear regression line sales on TV"
		tv_lm = lm(@formula(sales ~ TV), advertising)
		global tv_coef = coef(tv_lm)
		x = minimum(advertising.TV):maximum(advertising.TV)
        ax = Axis(fig[1, 1]; title, xlabel="TV", ylabel)
        scatter!(advertising.TV, advertising.sales)
		lines!(x, tv_coef[1] .+ tv_coef[2] * x; color=:darkred)
 
       	title = "Linear regression line sales on radio"
		radio_lm = lm(@formula(sales ~ radio), advertising)
		global radio_coef = coef(radio_lm)
		x = minimum(advertising.radio):maximum(advertising.radio)
        ax = Axis(fig[1, 2]; title, xlabel="radio", ylabel)
        scatter!(advertising.radio, advertising.sales)
		lines!(x, radio_coef[1] .+ radio_coef[2] * x; color=:darkred)
		
        title = "Linear regression line sales on newspaper"
		newspaper_lm = lm(@formula(sales ~ newspaper), advertising)
 		global newspaper_coef = coef(newspaper_lm)
		x = minimum(advertising.newspaper):maximum(advertising.newspaper)
    	ax = Axis(fig[1, 3]; title, xlabel="newspaper", ylabel)
        scatter!(advertising.newspaper, advertising.sales)
		lines!(x, newspaper_coef[1] .+ newspaper_coef[2] * x; color=:darkred)
    end

    fig
end

# ╔═╡ deafe35e-291e-4047-82cf-ea46a79e35b2
[tv_coef, radio_coef, newspaper_coef]

# ╔═╡ b7445e96-fb66-4721-a3d2-d26b032a3acf
md" ##### Below an example of using Stan."
...
model {
    ...
    sigma ~ exponential(1);
    // Compute mu
    mu= a + b[1] * tv + b[2] * radio + b[3] * newspaper;
    // LogLikelihood
    sales ~ normal(mu, sigma);
}";

# ╔═╡ 268f9c77-ca93-4816-b429-a5176c00fff4
let
    data = (N = nrow(advertising), tv = advertising.TV, radio = advertising.radio, newspaper = advertising.newspaper, sales = advertising.sales)
    global m2_2s = SampleModel("m2_2s", stan2_2)
    global rc2_2s= stan_sample(m2_2s; data)
    success(rc2_2s) && describe(m2_2s, [:a, Symbol("b[1]"),  Symbol("b[2]"), Symbol("b[3]"),:sigma])
end

# ╔═╡ ec732779-081e-4390-b69b-b8bbf3e410b5
if success(rc2_2s)
    post2_2s = read_samples(m2_2s, :nesteddataframe)
end

# ╔═╡ f96aa55c-a603-429c-964c-cb8f193b9ca1
b = nested_column_to_array(post2_2s, :b)
